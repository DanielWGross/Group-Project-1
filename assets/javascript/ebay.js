    ...
    callAPI: function(queryTerm) {
        $.ajax({
            ...
            dataType: 'json'
                ...
                ebay.results.URL.push(siteURL);
                var title = itemInfo.title[0];
                ebay.results.name.push(title);
                ebay.results.description.push(title);
                ...
                $.ajax({
                  ...
                  .then(function(response){ 
                    ...
                    ebay.results.image.push(imageURL);
                    var price = response.Item[0].ConvertedCurrentPrice.Value;
                    ebay.results.price.push(price);
                  }); 
                  if (ebay.results.name.length === 5) {
                    storage.ebay.response = ebay.results;
